...
contract MyStaking is Ownable{
    ...
    uint256 public  STAKING_DURATION = 10;
    ...
    function unstake() external {
        require(block.timestamp > stakingTime[msg.sender]+STAKING_DURATION, "Staking time not reached");
        require(stakers[msg.sender] > 0, "No balance to unstake");
        require(istokenstake(msg.sender),"Not a staker");
        uint256 amount = stakers[msg.sender];
        uint256 rewardAmount = amount.mul(rewardRate).div(100); // 10% reward
        token.transfer(msg.sender,amount.add(rewardAmount));
        stakers[msg.sender] = 0;
       
        emit Unstaked(msg.sender, stakers[msg.sender]);
    }
    
    function setStakingDuration(uint256 _stakingDuration) external onlyOwner {
        STAKING_DURATION = _stakingDuration;
    }
